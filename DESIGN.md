# Home Tech Solutions & Services

# DESIGN SYSTEM

Version: 1.0

---

# Design Philosophy

The website should feel:

- Professional
- Modern
- Clean
- Reliable
- Premium
- Easy to read

The design is based on the existing Google Sites website and will be modernized while keeping its familiar layout.

---

# Brand

Company Name

Home Tech Solutions & Services

Tagline

Repair • Install • Connect

Service Area

County Mayo

---

# Logo

Primary Logo

White version

Location:
images/logo/logo-white.png

Secondary Logo

Black version

Location:
images/logo/logo-black.png

Favicon

Location:
favicon/favicon.png

---

# Colour Palette

Primary

#111111

Accent Gold

#C9A227

White

#FFFFFF

Light Background

#F5F5F5

Dark Background

#1F1F1F

Text

#333333

Border

#E5E5E5

Success

#198754

Danger

#DC3545

---

# Typography

Headings

Poppins

Weight

700

Body

Inter

Weight

400

Navigation

500

Buttons

600

---

# Layout

Maximum Width

1200px

Section Padding

100px

Container Padding

20px

Border Radius

12px

Card Radius

16px

Button Radius

10px

---

# Buttons

Primary Button

Background

Black

Text

White

Hover

Gold

Secondary Button

Background

White

Border

Black

Hover

Light Grey

---

# Icons

Bootstrap Icons

Style

Outline

---

# Navigation

Logo

Home Tech Solutions & Services

Menu

Home

Services

Projects

About

Contact

Quote

Sticky Navigation

Yes

Transparent on Hero

No

---

# Hero

Background

Large high-quality photo

Overlay

Dark 40%

Title

Professional Home Improvement
Across County Mayo

Subtitle

Reliable handyman, furniture assembly,
TV mounting, ventilation, CCTV,
Wi-Fi & smart home services.

Buttons

Get Free Quote

View Services

---

# Services

Style

Cards

Columns

3 Desktop

2 Tablet

1 Mobile

Hover

Lift

Shadow

Scale Image

---

# Images

Use only:

- Real projects
- Own photographs
- Professional stock images only when necessary

Avoid:

- Low quality images
- Watermarked images
- Clipart

---

# Cards

Shadow

Small

Border Radius

16px

Padding

30px

---

# Footer

Dark background

White text

Contains:

Company

Phone

Email

WhatsApp

Google Business

Facebook

Privacy Policy

Copyright

---

# Responsive Breakpoints

Desktop

1200px+

Laptop

992px

Tablet

768px

Mobile

576px

---

# Animations

Fade In

Slide Up

Smooth Scroll

Hover Effects

Animation Speed

300ms

---

# Future Design Features

Dark Mode

Project Gallery

Before / After Slider

Testimonials Slider

Quote Calculator

Live Chat

Booking Calendar

Blog

---

# Design Rules

✓ Use plenty of white space

✓ Keep sections clean

✓ Maximum readability

✓ Real project photos

✓ Consistent spacing

✓ Minimal animations

✓ Fast loading

✓ Mobile first

✓ Accessibility friendly

✓ SEO friendly

---

# Inspiration

Current Google Sites website

Apple

Tesla

Modern service businesses

Minimal Scandinavian design